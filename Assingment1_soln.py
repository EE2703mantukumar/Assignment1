...
#  we are using "argv" function for accepting arguments from commandline. We are using "sys" module which contains that function.
...
# Note: we could have also used 'with' statement which is more convenient as in that we dont have to think of closing the file
        ...
        for line in lines:              # extracting circuit definition start and end lines
            if CIRCUIT == line[:len(CIRCUIT)]:# slicing is done for the line string in order to check when to start fetching each line. To avoid junks statements ithis command is used.
